...
TreeNode::TreeNode(NodeType nodeType, int lineno)
{
    ...
    label.begin_label = label.false_label = label.next_label = label.true_label = "";
}
...
void TreeNode::type_check()
{
    ...
    case NODE_EXPR:
        ...
        case OP_ADD:
            ...
            break;
        case OP_SUB:
            if (child[0]->type != child[1]->type || child[0]->type != VAR_INTEGER)
...
void tree::stmt_get_label(TreeNode *t)
{
    ...
    case STMT_COMPOUND:
        TreeNode *last;
        TreeNode *p;
        for (p = t->child[0]; p->nodeType == NODE_STMT && p->stmtType == STMT_DECL; p = p->sibling);
    ...
    case STMT_FOR:
        ...
        s3->label.begin_label = s1->label.true_label; //5
    ...
    case STMT_IF:
        ...
        { // if(e) s1
            ...
            if (t->label.next_label == "")
                t->label.next_label = new_label();
            e->label.false_label = t->label.next_label;
            s1->label.begin_label = e->label.true_label; //5
...
void tree::stmt_gen_code(ostream &out, TreeNode *t)
{
    ...
    case STMT_SCANF:
        ...
        //}
        // else{
        //     out << "\tpushl STR" << t->const_str << endl;
        //     out << "call scanf" << endl;
        //     out << "\taddl $" << 4<< ", %esp" << endl;
        // }
        break;
...
void tree::expr_gen_code(ostream &out, TreeNode *t)
{
    ...
    case OP_DIV:
        ...
        if (e1->nodeType == NODE_VAR)
            ...
            out << "t" << e1->temp_var;
        out << ", %eax" << endl;
        out << "\tcmp %eax, %edx" << endl; // 判断被除数是否大于0
        out << "\tjnb O"<< m << endl;
        out << "\tmovl $0, %edx" << endl;
        out << "\tnot %edx" << endl;
        out << "O"<< m << ":" <<endl;
