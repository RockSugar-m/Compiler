...
- `make test`命令会执行编译流程 并将`test`文件夹下的所有c文件逐个作为输入 将语法树输出结果写到对应的.res文件中。并在主目录下生成一个out.s的汇编代码，有多个测试文件时会覆盖
- `make clean`清理生成的文件
- 在主目录下使用gcc`gcc 1.c -m32 -o 1.s`编译汇编代码，生成可执行文件然后运行
- 汇编使用Linux平台的AT&T格式
...
# 类型检查
对于C代码中的错误将会在终端打印，支持变量未声明、重声明、变量在声明前使用、类型错误检查(比如字符串、布尔值不能参与某些运算、输入输出函数也有参数类型要求)等
